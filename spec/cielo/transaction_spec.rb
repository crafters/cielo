...
describe Cielo::Transaction do
  ...
  describe 'create a buy page store transaction with token generation' do
    before do
      stub_request(:any, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: '<?xml version="1.0" encoding="ISO-8859-1"?><transacao versao="1.2.1" id="1390315327" xmlns="http://ecommerce.cbmp.com.br"><tid>10069930690DCC341001</tid><pan>52WC7RsmcNuEUSjrYWAEhCOjoLMnMCm4KMTQBqN7PdM=</pan><dados-pedido><numero>1</numero><valor>100</valor><moeda>986</moeda><data-hora>2014-01-21T12:42:08.865-02:00</data-hora><idioma>PT</idioma><taxa-embarque>0</taxa-embarque></dados-pedido><forma-pagamento><bandeira>visa</bandeira><produto>1</produto><parcelas>1</parcelas></forma-pagamento><status>6</status><autenticacao><codigo>6</codigo><mensagem>Transacao sem autenticacao</mensagem><data-hora>2014-01-21T12:42:08.872-02:00</data-hora><valor>100</valor><eci>7</eci></autenticacao><autorizacao><codigo>6</codigo><mensagem>Transa??o autorizada</mensagem><data-hora>2014-01-21T12:42:08.885-02:00</data-hora><valor>100</valor><lr>00</lr><arp>123456</arp><nsu>904244</nsu></autorizacao><captura><codigo>6</codigo><mensagem>Transacao capturada com sucesso</mensagem><data-hora>2014-01-21T12:42:08.912-02:00</data-hora><valor>100</valor></captura><token><dados-token><codigo-token>2ta/YqYaeyolf2NHkBWO8grPqZE44j3PvRAQxVQQGgE=</codigo-token><status>1</status><numero-cartao-truncado>401288******1881</numero-cartao-truncado></dados-token></token></transacao>')
  ...
  describe 'create a buy page cielo transaction' do
    ...
    [:numero, :valor, :bandeira, :"url-retorno"].each do |parameter|
      it "raises an error when #{parameter} isn't informed" do
        expect { @transaction.create! @params.except!(parameter) }.to raise_error(Cielo::MissingArgumentError)
      end
    end

    it 'delivers an successful message' do
      stub_request(:post, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: '<?xml version="1.0" encoding="UTF-8"?><transacao id="1453217284" versao="1.2.1"><tid>100173489800002FDD0A</tid><dados-pedido><numero>1</numero><valor>100</valor><moeda>986</moeda><data-hora>2016-01-19T13:32:20.908-02:00</data-hora><idioma>PT</idioma><taxa-embarque>0</taxa-embarque></dados-pedido><forma-pagamento><bandeira>visa</bandeira><produto>1</produto><parcelas>1</parcelas></forma-pagamento><status>0</status><url-autenticacao>https://qasecommerce.cielo.com.br/web/index.cbmp?id=ef6ba5ecac12ab42bbdf84957912ff4a</url-autenticacao></transacao>')
    ...
    it "delivers a response with error message when status isn't 200" do
      stub_request(:any, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: "Nothing to be found 'round here", status: ['404', 'Not Found'])
      response = @transaction.create! @params

      expect(response[:erro]).to_not be_nil
      expect(response[:erro][:codigo]).to be_eql('000')
    end

    it 'delivers a response with error message when the server send' do
      stub_request(:any, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?> <erro xmlns=\"http://ecommerce.cbmp.com.br\"> <codigo>001</codigo> <mensagem>Requisição inválida</mensagem> </erro>")
      response = @transaction.create! @params

      expect(response[:erro]).to_not be_nil
      expect(response[:erro][:codigo]).to be_eql('001')
    end
  end

  describe 'Verify a transaction status' do
    it 'returns null when no tid is informed' do
      expect(@transaction.verify!(nil)).to be_nil
    end
    it 'returns a successfull message' do
      stub_request(:any, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n<transacao id=\"1308170973\" versao=\"1.1.0\" xmlns=\"http://ecommerce.cbmp.com.br\">\n  <tid>1001734898056B3C1001</tid>\n  <dados-pedido>\n    <numero>1</numero>\n    <valor>100</valor>\n    <moeda>986</moeda>\n    <data-hora>2011-06-15T18:45:16.705-02:00</data-hora>\n    <idioma>PT</idioma>\n  </dados-pedido>\n  <forma-pagamento>\n    <bandeira>visa</bandeira>\n    <produto>1</produto>\n    <parcelas>1</parcelas>\n  </forma-pagamento>\n  <status>0</status>\n</transacao>\n\n")
  ...
  describe 'Catch a transaction' do
    it 'returns null when no tid is informed' do
      expect(@transaction.catch!(nil)).to be_nil
    end
    it 'returns a successfull message' do
      stub_request(:any, 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do')
        .to_return(body: "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n<transacao id=\"1308170973\" versao=\"1.1.0\" xmlns=\"http://ecommerce.cbmp.com.br\">\n  <tid>1001734898056B3C1001</tid>\n  <dados-pedido>\n    <numero>1</numero>\n    <valor>100</valor>\n    <moeda>986</moeda>\n    <data-hora>2011-06-15T18:45:16.705-02:00</data-hora>\n    <idioma>PT</idioma>\n  </dados-pedido>\n  <forma-pagamento>\n    <bandeira>visa</bandeira>\n    <produto>1</produto>\n    <parcelas>1</parcelas>\n  </forma-pagamento>\n  <status>0</status>\n</transacao>\n\n")
